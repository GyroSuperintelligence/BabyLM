...
import os
from typing import Optional, Dict, List, Iterable, Union, BinaryIO
from pathlib import Path

# The ExtensionManager is the workhorse orchestrated by this API
from .extension_manager import ExtensionManager
from .gyro_errors import GyroSessionError, GyroError
...
def process_byte(session_id: str, byte: int) -> Optional[Tuple[int, int]]:
    """
    Processes a single byte through the navigation cycle.
    
    Args:
        session_id: The ID of the active session to use.
        byte: The byte value (0-255) to process.
...
def process_file(session_id: str, file_path: Union[str, Path], 
...
def import_knowledge(bundle_path: Union[str, Path], 
...
def query_memory(session_id: str, tag: str) -> any:
    ...
    if "genetic" in tag or any(inv in tag for inv in 
                              ["gyrotensor_id", "gyrotensor_com", "gyrotensor_nest", 
                               "gyrotensor_add", "gyrotensor_quant"]):
...
def get_navigation_history(session_id: str, count: int = 100, 
