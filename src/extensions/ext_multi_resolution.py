...
class ext_MultiResolution(GyroExtension):
    ...
    def ext_on_navigation_event(self, nav_event: int, byte: Optional[int] = None) -> None:
        """
        Process navigation event for boundary detection.
        
        Args:
            nav_event: Packed navigation byte (two 4-bit codes)
            byte: Original input byte that caused this navigation
        """
    ...
    def _record_boundary(self, boundary_type: str, defect_value: int, byte: Optional[int]) -> None:
    ...
    def ext_get_text_structure(self) -> Dict[str, Any]:
        ...
        if self._boundary_stats['word_count'] == 0:
            ...
            avg_word_length = self._boundary_stats['char_count'] / self._boundary_stats['word_count']
        
        if self._boundary_stats['sent_count'] == 0:
            avg_sent_length = 0
        else:
            avg_sent_length = self._boundary_stats['word_count'] / self._boundary_stats['sent_count']
