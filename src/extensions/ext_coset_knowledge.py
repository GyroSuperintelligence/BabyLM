...
class ext_CosetKnowledge(GyroExtension):
    ...
    def _update_compression_ratio(self) -> None:
        ...
        original_size = sum(
            len(zlib.decompress(compressed)) for compressed in self._pattern_store.values()
        )
    ...
    def get_learning_state(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "coset_structure": {rep: list(members) for rep, members in self._coset_members.items()},
    ...
    def ext_on_navigation_event(self, nav_event: int, input_byte: Optional[int] = None) -> None:
        ...
        if len(self._pattern_window) >= 8:
            ...
            pattern_bytes = bytes(
                [
                    event[0]
                    for event in self._pattern_window[-8:]
