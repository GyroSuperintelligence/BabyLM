...
class ext_PerformanceTracker(GyroExtension):
    ...
    def __init__(self):
        ...
        self._component_metrics = defaultdict(lambda: {
            'call_count': 0,
            'total_time': 0.0,
            'min_time': float('inf'),
            'max_time': 0.0,
    ...
    def end_operation(self, resonated: bool = False, error: Optional[Exception] = None) -> float:
        ...
        if duration_ms > self._alert_thresholds['operation_time_ms']:
            self._record_alert('slow_operation', {
    ...
    def _update_throughput(self) -> None:
        ...
        if len(self._throughput_window) > 1:
            ...
            if time_span > 0:
                self._system_metrics['operations_per_second'] = len(self._throughput_window) / time_span
        
        # Update resonance rate
        if self._system_metrics['total_operations'] > 0:
            self._system_metrics['resonance_rate'] = (
                self._system_metrics['total_resonances'] / 
                self._system_metrics['total_operations']
            )
    ...
    def ext_get_operation_stats(self, operation_name: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'p95_time_ms': sorted(timings)[int(len(timings) * 0.95)] if len(timings) > 20 else max(timings),
    ...
    def ext_get_performance_report(self) -> Dict[str, Any]:
        ...
        report['bottlenecks'] = sorted(bottlenecks, key=lambda x: x['average_time_ms'], reverse=True)
