...
def show_welcome():
    ...
    console.print("  [green]python gyro_tools/gyro_cli.py[/green]                      # Interactive mode")
    console.print("  [green]python gyro_tools/gyro_cli.py knowledge list[/green]       # List knowledge packages")
    console.print("  [green]python gyro_tools/gyro_cli.py session export --session-id <ID> --output mysession.session.gyro[/green]")
    console.print("  [green]python gyro_tools/gyro_cli.py system health[/green]        # Show system health")
...
def main():
    ...
    k_import.set_defaults(
        func=lambda args: knowledge.import_package(args.input)
    )
    ...
    k_fork.set_defaults(
        func=lambda args: knowledge.fork_package(args.source, args.session)
    )
    ...
    k_info.set_defaults(
        func=lambda args: knowledge.show_package_info(args.knowledge_id)
    )
    ...
    k_delete.set_defaults(
        func=lambda args: knowledge.delete_package(args.knowledge_id)
    )
    ...
    s_export.set_defaults(
        func=lambda args: session.export_session(args.session_id, args.output)
    )
    ...
    s_import.set_defaults(
        func=lambda args: session.import_session(args.input)
    )
    ...
    s_link.set_defaults(
        func=lambda args: session.link_to_knowledge(args.session, args.knowledge)
    )
    
    s_info = s_sub.add_parser("info", help="Show info for a session")
    s_info.add_argument("--session-id", required=True, help="Session UUID")
    s_info.set_defaults(
        func=lambda args: session.show_session_info(args.session_id)
    )
    ...
    s_delete.set_defaults(
        func=lambda args: session.delete_session(args.session_id)
    )
    ...
    i_know.set_defaults(
        func=lambda args: integrity.check_knowledge(args.knowledge_id)
    )
    ...
    i_sess.set_defaults(
        func=lambda args: integrity.check_session(args.session_id)
    )
    ...
    curriculum_parser.set_defaults(
        func=lambda args: (
            curriculum.list_resources() if args.list
            else curriculum.download_resource(args.download, args.dest) if args.download
