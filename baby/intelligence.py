...
from .inference import EndogenousInferenceOperator
from .types import (
    PhenotypeStore, 
    AgentConfig, 
    CycleHookFunction, 
...
class IntelligenceEngine:
    ...
    def __init__(
        self, 
        manifold_path: str, 
        phenotype_store: PhenotypeStore, 
        ...
        self.s2 = InformationEngine(manifold_data)
    ...
    def process_egress(self, input_byte: int) -> int:
        ...
        self.gene_mac_m_int = governance.apply_gyration_and_transform(
            self.gene_mac_m_int, intron
        )
    ...
    def process_ingress(self, last_intron: int) -> int:
        ...
        if isinstance(phenotype, str):
            return ord(phenotype[0]) if phenotype else ord('?')
        else:
    ...
    def batch_learn(self, data: bytes) -> None:
        ...
        path-dependent nature of the coaddition operation.
        ...
        # Process all bytes to update physical state
        for byte in data:
        ...
        # Learn from the ordered reduction of all introns
        if introns:
            learning_intron = governance.batch_introns_coadd_ordered(introns)
            phenotype_entry = self.operator.get_phenotype(
                self.gene_mac_m_int, learning_intron
            )
...
class GyroSI:
    ...
    def __init__(
        ...
        config: AgentConfig, 
        agent_id: Optional[str] = None, 
        phenotype_store: Optional[PhenotypeStore] = None
    ):
        """
    ...
    def apply_maintenance(
        ...
        pruned_count = self.engine.operator.prune_low_confidence_entries(
    ...
    def _create_default_store(self) -> PhenotypeStore:
        ...
        # Create base store
        if 'public_knowledge_path' in self.config:
            # Multi-agent setup with public/private knowledge
            private_path = self.config.get(
                'private_knowledge_path',
                f'memories/private/agents/{self.agent_id}/knowledge.pkl.gz'
            )
            
            base_store = MultiAgentPhenotypeStore(
                public_store_path=self.config['public_knowledge_path'],
                private_store_path=private_path
            )
        else:
            # Single-agent setup
            knowledge_path = self.config.get(
                'knowledge_path',
                'memories/knowledge.pkl.gz'
            )
            base_store = PickleStore(knowledge_path)
        
        # Wrap with canonicalizing decorator if enabled
        if enable_canonical:
            canonical_map_path = self.config.get(
                'canonical_map_path',
                'memories/public/manifold/canonical_map.json'
            )
            if os.path.exists(canonical_map_path):
                return CanonicalizingStore(base_store, canonical_map_path)
            else:
                print(f"Warning: Canonical map not found at {canonical_map_path}")
                print("Falling back to non-canonical storage")
...
class AgentPool:
        ...
        with self._lock:
            ...
            if agent_id not in self.agents:
                ...
                config: AgentConfig = {
                    ...
                    "enable_canonical_storage": self.preferences.get(
    ...
    def _maybe_evict_agent(self) -> None:
        ...
        if len(self.agents) >= self.max_agents:
            if self.eviction_policy == 'lfu':
                ...
                oldest_id = min(
                    self.agent_access_times,
                    key=self.agent_access_times.get
                )
            else:
    ...
    def _evict_expired_agents(self) -> None:
        ...
        expired_agents = [
            agent_id for agent_id, last_access in self.agent_access_times.items()
