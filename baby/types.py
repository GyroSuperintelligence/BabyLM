...
class PhenotypeStore(Protocol):
    ...
    Note:
        ...
        All persistent backends should hash or pack this key
        to a fixed size for efficient storage operations.
    """
    
    def get(self, context_key: Tuple[int, int]) -> Optional[Dict[str, Any]]:
        """Retrieve phenotype entry for given context key."""
        ...
    
    def put(self, context_key: Tuple[int, int], entry: Dict[str, Any]) -> None:
        """Store phenotype entry for given context key."""
        ...
    
    def close(self) -> None:
        """Clean shutdown and persist any pending changes."""
        ...
