        ...
        for i in range(0, len(weight_keys), chunk_size):
            ...
            if debug:
                print(f"    Saved chunk {i//chunk_size + 1}/"
                      f"{(len(weight_keys) + chunk_size - 1)//chunk_size}")
...
class GyroKernel:
    ...
    def __init__(
        ...
        if self.debug:
            ...
            print(f"📍 CS state: index={self.CS_STATE_INDEX}, "
                  f"θ={float(self.theta[self.CS_STATE_INDEX]):.4f}")
    ...
    def _precompute_una_pool(self) -> None:
        ...
        self._UNA_pool = np.argwhere(
            np.abs(self.theta - target_theta) < tolerance
        ).astype(np.int32).ravel()
        
        if len(self._UNA_pool) == 0:
            # Fallback: use states with theta in UNA range
            self._UNA_pool = np.argwhere(
                (self.theta > THETA_CS) & (self.theta < THETA_ONA)
            ).astype(np.int32).ravel()
    ...
    def _set_special_tokens(self) -> None:
        ...
        if self.debug:
            print(f"📌 Special tokens: CLS={self.CLS_TOKEN}, SEP={self.SEP_TOKEN}, "
    ...
    def _project_embeddings_to_token_states(self, embeddings: np.ndarray) -> None:
        ...
        if cache_file.exists() and not self.force_reconvert:
            try:
                ...
                if self.debug:
                    print(f"✅ Loaded cached embedding projection: "
                          f"{len(self.token_state_map)} tokens")
                return
            except Exception:
                pass
    ...
    def _build_or_load_token_post_states(self) -> None:
        ...
        if post_path.exists() and exons_path.exists():
            ...
            self.token_post_state_index = {
                int(i): int(s) for i, s in enumerate(post_idx) if s >= 0
            }
            self.token_exon_cache = {
                int(i): int(e) for i, e in enumerate(exons) if e >= 0
            }
    ...
    def token_to_introns(self, token_id: int) -> List[int]:
        ...
        if token_id >= self.tokenizer.vocab_size: return []
    ...
    def _apply_intron_and_gate(self, state_index: int, intron: int) -> int:
        ...
        if 0 <= state_index < len(self.epistemology):
            ...
            if next_stage_idx < current_stage_idx:
                if self.debug:
                    print(f"  ↪️ Cycle gating blocked regression: {STAGE_ORDER[current_stage_idx]} → {STAGE_ORDER[next_stage_idx]}")
                return state_index
    ...
    def _seed_from_prompt(self, text: str) -> None:
        ...
        for token_id in tokens:
            if token_id in self.valid_tokens:
                introns = self.token_to_introns(token_id)
                for intron in introns:
                    self.current_state_index = self._apply_intron_and_gate(
                        self.current_state_index, intron
                    )
                    self.path_memory = fold(self.path_memory, intron)
    ...
    def generate_token(self) -> Optional[int]:
        ...
        if not candidates:
            ...
            populated_orbits = {
                ...
                for orb, cands in self._orbit_candidates.items() if cands
            }
            if not populated_orbits: return self.SEP_TOKEN
