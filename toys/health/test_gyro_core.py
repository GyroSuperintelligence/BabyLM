...
def test_psi_boundary_leb128():
    ...
    # Verify they match
    assert local_leb128 == engine_leb128, \
        f"Local and engine LEB128 encodings differ: {local_leb128} vs {engine_leb128}"
    ...
    for i, byte_val in enumerate(local_leb128):
        ...
        if i < len(local_leb128) - 1:  # Non-final bytes
            ...
            assert (byte_val & 0x80) == 0x80, \
                f"Non-final byte {i} should have MSB=1: 0x{byte_val:02X}"
            assert (intron & 0x80) == 0x00, \
                f"Non-final intron {i} should have MSB=0 after boundary transform: 0x{intron:02X}"
            ...
            assert (byte_val & 0x80) == 0x00, \
                f"Final byte should have MSB=0: 0x{byte_val:02X}"
            assert (intron & 0x80) == 0x80, \
                f"Final intron should have MSB=1 after boundary transform: 0x{intron:02X}"
    ...
    assert recovered_bytes == local_leb128, \
...
def test_global_channel_monotonicity():
    ...
    for k in range(len(micro_path) - 1):
        ...
        assert next_alignment >= current_alignment, \
...
def test_inadmissible_token_rejection():
    ...
    assert not engine.is_admissible(start_state, token_id, global_strict=False), \
        ...
        try:
            ...
            for slab_idx in range(8):  # 8 slabs
                ...
                if min_slab < initial_slab and final_slab >= initial_slab:
                    print(f"Token {token_id}, slab {slab_idx}: dips from {initial_slab} to {min_slab}, ends at {final_slab}")
                    found_dipping_token = True
                    
                    # Verify slab admissibility (should pass - only checks initial vs final)
                    assert final_slab >= initial_slab, \
                    ...
                    stepwise_would_fail = any(slab_alignments[i+1] < slab_alignments[i] 
                                            for i in range(len(slab_alignments)-1))
...
def test_tie_breaking_determinism():
    ...
    assert lex_keys != sorted_keys or len(set(lex_keys)) == len(lex_keys), \
...
def test_bit_packing_invariants():
    ...
    for layer in range(4):  # 4 layers (0-3)
        for frame in range(2):  # 2 frames (0-1)
            try:
                ...
                assert all(0 <= pos < 48 for pos in positions), \
                    f"Invalid bit positions: {positions}"
                
                # Verify positions are unique within this slab
                assert len(set(positions)) == len(positions), \
                    f"Duplicate positions in slab: {positions}"
    ...
    expected_total = 8 * 6
    assert total_bits_checked == expected_total, \
        f"Expected {expected_total} total bits, checked {total_bits_checked}"
...
def test_reverse_index_requirement():
    ...
    for state in test_states:
        ...
        assert engine.ontology_keys[index] == state, \
        ...
        if hasattr(engine, method_name):
            ...
            try:
                ...
                assert 'np.where' not in source, \
                    f"Method {method_name} contains np.where - potential linear search"
                assert 'numpy.where' not in source, \
...
def test_versioning_integrity():
    """Test validate_maps() for version mismatch detection and address recompute."""
    print("Testing versioning integrity...")
    
    engine = create_real_engine()
    
    # Test current version validation (should pass)
    try:
        result = engine.validate_maps()
        assert result == True, "validate_maps() should return True for valid maps"
        print("[OK] Current version validation passed")
    except Exception as e:
        print(f"Current version validation failed: {e}")
        # This might be expected if versions are mismatched
    
    # Test version validation method
    try:
        result = engine.validate_versions()
        assert result == True, "validate_versions() should return True for valid versions"
        print("[OK] Version validation passed")
    except Exception as e:
        print(f"Version validation failed (may be expected): {e}")
    
    # Test that version info is present
    assert hasattr(engine, 'version_info'), "Missing version_info"
    assert isinstance(engine.version_info, dict), "version_info should be a dictionary"
    
    required_versions = ['atlas_version', 'address_version', 'config_version']
    for version_key in required_versions:
        assert version_key in engine.version_info, f"Missing {version_key} in version_info"
        print(f"[OK] {version_key}: {engine.version_info[version_key]}")
    
    print("[OK] Versioning integrity verified")
    ...
    try:
        ...
        if state_idx is not None:
            ...
            if memory_key in engine.passive_memory_index:
                ...
                print(f"[OK] Passive memory entry: touch_count={entry['touch_count']}, zero_streak={entry['zero_streak']}")
                
                # Verify touch_count is within valid range
                assert 0 <= entry['touch_count'] <= 255, \
                    f"touch_count out of range: {entry['touch_count']}"
...
def test_address_binding_medoid():
    ...
    if len(engine.orbit_representatives) > 0:
        ...
        for token_id in test_tokens:
            try:
                ...
                assert address in engine.state_to_index, \
                    f"Address {address:012X} not found in state index"
                
                # Verify address is deterministic (call twice)
                address2 = engine.address_of_token(token_id)
                assert address == address2, \
                    f"Address not deterministic: {address:012X} != {address2:012X}"
    ...
    assert test_address <= engine.MASK48, \
        f"Address exceeds 48-bit mask: 0x{test_address:012X}"
