...
class TestPhysicsInvariants:
    ...
    def test_transformation_determinism(self):
        ...
        results = [
            governance.apply_gyration_and_transform(state, intron)
            for _ in range(10)
        ]
...
class TestPerformance:
    ...
    def test_transformation_performance(self, generate_test_introns):
        """Benchmark transformation operations."""
        from conftest import Timer
        
        state = 0
        introns = generate_test_introns(10000)
        
        with Timer() as timer:
            for intron in introns:
                state = governance.apply_gyration_and_transform(state, intron)
        
        # Should process at least 100k transformations per second
        ops_per_second = len(introns) / timer.elapsed
        assert ops_per_second > 100_000, f"Only {ops_per_second:.0f} ops/sec"
    
    def test_coaddition_performance(self, generate_test_introns):
        """Benchmark coaddition operations."""
        from conftest import Timer
        ...
        # Should process at least 100k coadditions per second
        ops_per_second = len(introns) / timer.elapsed
        assert ops_per_second > 100_000, f"Only {ops_per_second:.0f} ops/sec"