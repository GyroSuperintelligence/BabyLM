...
from baby.information import InformationEngine, PickleStore
...
class TestEndogenousInferenceOperator:
    ...
    @pytest.fixture
    def inference_operator(self, manifold_data, pickle_store):
        """Create an inference operator for testing."""
        _, mock_manifold = manifold_data
        s2_engine = InformationEngine(mock_manifold)
        return EndogenousInferenceOperator(s2_engine, pickle_store)
    ...
    def test_get_phenotype_new(self, inference_operator):
        ...
        phenotype = inference_operator.get_phenotype(state_int, intron)
    ...
    def test_get_phenotype_existing(self, inference_operator):
        ...
        phenotype1 = inference_operator.get_phenotype(state_int, intron)
        ...
        phenotype2 = inference_operator.get_phenotype(state_int, intron)
...
class TestKnowledgeManagement:
    ...
    def test_validate_integrity(self, populated_operator):
        ...
        assert report["store_type"] == "PickleStore"
    ...
    def test_confidence_decay(self, populated_operator, mock_time):
        ...
        report = populated_operator.apply_confidence_decay(
            decay_factor=0.99,
            age_threshold=5,
        ...
        assert old_entry["confidence"] < 0.5  # Should have decayed
    ...
    def test_prune_low_confidence(self, populated_operator):
        ...
        pruned = populated_operator.prune_low_confidence_entries(
...
class TestLearningMechanics:
    ...
    @pytest.fixture
    def operator_with_state(self, manifold_data, pickle_store):
        """Create operator with specific test state."""
        _, mock_manifold = manifold_data
        s2_engine = InformationEngine(mock_manifold)
        operator = EndogenousInferenceOperator(s2_engine, pickle_store)
...
class TestPerformance:
    ...
    def test_phenotype_lookup_performance(self, inference_operator):
        """Test phenotype lookup performance."""
        from conftest import Timer
        
        # Pre-populate some entries
        for i in range(1000):
            inference_operator.get_phenotype(i % 100, i % 256)
        ...
        ops_per_second = lookups / timer.elapsed
        assert ops_per_second > 10000, f"Only {ops_per_second:.0f} lookups/sec"
    
    def test_learning_performance(self, inference_operator):
        """Test learning operation performance."""
        from conftest import Timer
        
        # Get a phenotype
        phenotype = inference_operator.get_phenotype(42, 137)
        
        # Measure learning performance  
        iterations = 10000
        with Timer() as timer:
            for i in range(iterations):
                inference_operator.learn(phenotype, i % 256)
        
        ops_per_second = iterations / timer.elapsed
        assert ops_per_second > 10000, f"Only {ops_per_second:.0f} learns/sec"