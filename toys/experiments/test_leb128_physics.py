...
from baby.leb128_physics import (
    ψ,
    ψ_inv,
    encode_token_to_leb128,
    decode_leb128_to_token,
    token_to_introns,
    introns_to_token,
    TokenSTT,
    MinimalPhenotype,
    text_to_intron_stream,
    intron_stream_to_text,
    apply_token_physics,
...
def test_minimal_phenotype():
    ...
    phenotype = MinimalPhenotype(state_index=12345, token_id=67890, exon_mask=0xAA, confidence=0.85)

    print(
        f"Original: state={phenotype.state_index}, token={phenotype.token_id}, "
    ...
    print(
        f"Restored: state={restored.state_index}, token={restored.token_id}, "
