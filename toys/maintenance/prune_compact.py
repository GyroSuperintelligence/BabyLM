...
def main() -> None:
    ...
    for path in (args.store, args.output):
        if path and (path.endswith(".pkl") or path.endswith(".pkl.gz")):
            print(f"ERROR: This script only supports binary_struct-based stores (.bin). Refusing to run on: {path}")
            exit(1)
